...
#define BT_AVDTP_RSP_ACCEPT 0x02
#define BT_AVDTP_RSP_REJECT 0x03
...
#if BR_EDR_PTS_TEST
#define BT_L2CAP_CONFIG_STREAM    0x01
struct bt_avdtp_close_stream {
	struct bt_avdtp_req req;
	uint8_t status;
	struct bt_avdtp_sep *caps;
};

struct bt_avdtp_open_stream {
	struct bt_avdtp_req req;
	uint8_t status;
	struct bt_avdtp_sep *caps;
};

struct bt_avdtp_get_cap {
	struct bt_avdtp_req req;
	struct bt_avdtp_seid_info *caps;
};

struct bt_avdtp_sep_config_params {
	uint8_t acp_seid:6;
	uint8_t int_seid:6;
	uint16_t param_len;
	uint8_t *param;
...
struct bt_avdtp_set_conf {
	struct bt_avdtp_req req;
	struct bt_avdtp_sep_config_params sep_config_param;
};

struct bt_avdtp_start_stream {
	struct bt_avdtp_req req;
	uint8_t status;
	struct bt_avdtp_sep *caps;
};

struct bt_avdtp_suspend_stream {
	struct bt_avdtp_req req;
	uint8_t status;
	struct bt_avdtp_sep *caps;
};
...
#if BR_EDR_PTS_TEST
/* AVDTP Get Stream */
int bt_avdtp_get_cap(struct bt_avdtp *session,
		      struct bt_avdtp_get_cap *cap);

/* AVDTP Set Configuration */
int bt_avdtp_set_conf(struct bt_avdtp *session,
		      struct bt_avdtp_set_conf *para);

/* AVDTP Close Stream */
int bt_avdtp_close_stream(struct bt_avdtp *session,
		      struct bt_avdtp_req *req);

/* AVDTP Open Stream */
int bt_avdtp_open_stream(struct bt_avdtp *session,
		      struct bt_avdtp_req *req);

/* AVDTP Start Stram */
int bt_avdtp_start_stream(struct bt_avdtp *session,
		      struct bt_avdtp_start_stream *para);

int bt_avdtp_suspend_stream(struct bt_avdtp *session,
		      struct bt_avdtp_suspend_stream *para);

int bt_avdtp_report_delay(struct bt_avdtp *session,
		      struct bt_avdtp_req *req);
#endif
