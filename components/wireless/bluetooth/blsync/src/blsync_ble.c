                   ...
                   u16_t len, u16_t offset, u8_t flags)
{
    ...
    //printf_hexdump("data", (uint8_t *)buf, len);
...
static int __ble_bytes_send(void *p_drv, const void *buf, size_t bytes)
{
    ...
    //printf(" ble send size %d\r\n", bytes);

    //printf_hexdump("data", (uint8_t *)buf, bytes);
    //printf("\r\n");
    ...
    while (!index->stop_flag) {
        ...
        if (xQueueReceive(index->xQueue1,
            ...
            if (pro_trans_read(index->pro_handle, recv_buf.buf,
                ...
                printf("protocol analyze failed\r\n");
            }
...
{
	//printf_hexdump("vnd_sync_get \n", buf->data, buf->len);
#if 1
	if (!gp_index || gp_index->rbuf_len == 0) {
		printf("bl_blemesh_sync_start not call or len is zero\n");
		return ;
	}
...
static void vnd_sync_set(struct bt_mesh_model *model,
                ...
                struct net_buf_simple *buf)
{
	//printf_hexdump("vnd_sync_set \n", buf->data, buf->len);
#if 1
	if(!gp_index){
		printf("bl_blemesh_sync_start not call\n");
		return;
	}
	//gp_index->p_cur_conn = conn;

	printf("write length %d\r\n", buf->len);
	//printf_hexdump("data", buf->data, buf->len);
