...
bouffalo sdk not for bl602 wifi */
#elif defined(BL702)
    // {(uintptr_t)0x28008000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD, "EM_REG"},
#elif defined(BL702L)
    // {(uintptr_t)0x28008000, (unsigned int)&__LD_CONFIG_EM_SEL, DUMP_BASE64_WORD, "EM_REG"},
...
void bl_coredump_run() {
  ...
  for (cmd_pos = 0; (&_dump_sections + cmd_pos)->addr != 0xffffffff; cmd_pos++) {
    if ((&_dump_sections + cmd_pos)->addr == 0)
      break;
    if ((&_dump_sections + cmd_pos)->len == 0)
      continue;
    bl_coredump_print_n_k((&_dump_sections + cmd_pos)->addr, (&_dump_sections + cmd_pos)->len, "predefined", DUMP_BASE64_WORD);
  }
