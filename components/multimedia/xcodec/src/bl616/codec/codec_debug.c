...
#define USER_CONFIG_DEBUG  (0)

#if USER_CONFIG_DEBUG
#define user_log(M, ...) do {  printf("[%9ld]=====[%s:%d] " M "",     \
                                 (uint32_t)(bl_os_clock_gettime_ms()), \
                                 __func__,                             \
                                 __LINE__,                             \
                                 ##__VA_ARGS__);                       \
                               } while(0==1)
...
/* buf debug */
#define ENABLE_CODEC_DEBUG_OUTPUT   0
#define ENABLE_CODEC_DEBUG_INPUT    0
...
{
    {
        ...
        printf("XTAL[40M] -> aupll_refclk_sel[%d] -> AUPLL[442.368M] -> aupll_postdiv[div %d] -> audio_pll_clk[%f M]\r\n",
