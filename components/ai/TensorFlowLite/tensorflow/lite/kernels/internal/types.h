...
enum class PaddingType : uint8_t { kNone,
                                   kSame,
                                   kValid };
...
public:
    ...
    explicit RuntimeShape(int dimensions_count)
    ...
    RuntimeShape(const std::initializer_list<int> init_list)
        : size_(0)
    {
        BuildFrom(init_list);
    }
    ...
    // rolls out.
    RuntimeShape(RuntimeShape const &other)
        : size_(other.DimensionsCount())
    {
    ...
    inline int32_t DimensionsCount() const
    {
        return size_;
    }
    ...
    inline const int32_t *DimsDataUpTo5D() const
    {
        return dims_;
    }
