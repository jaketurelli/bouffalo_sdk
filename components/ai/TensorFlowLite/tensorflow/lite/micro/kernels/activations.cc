                                ...
                                ReluOpData *data)
{
    ...
    data->params.quantized_activation_max =
        act_max == std::numeric_limits<float>::infinity() ? static_cast<int32_t>(std::numeric_limits<T>::max()) : std::min(static_cast<int32_t>(std::numeric_limits<T>::max()), output->params.zero_point + static_cast<int32_t>(roundf(act_max / output->params.scale)));
                       ...
                       const RuntimeShape &output_shape, float *output_data)
{
    ...
    for (int i = 0; i < flat_size; ++i) {
        ...
        const float lower = 0.0f;
        const float clamped = val > upper ? upper : val < lower ? lower :
                                                                  val;
        output_data[i] = clamped;
    }
}
                           ...
                           const RuntimeShape &output_shape, Q *output_data)
{
    ...
    for (int i = 0; i < flat_size; ++i) {
        ...
        const Q clamped = val > upper ? upper : val < lower ? lower :
                                                              val;
        output_data[i] = clamped;
    }
}
