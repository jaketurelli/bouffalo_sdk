...
tIntegerType ShiftLeft(tIntegerType a, int offset)
{
    ...
    return wide_shifted < min ? min : wide_shifted > max ? max :
...
template <int Exponent, typename IntegerType,
          int ExponentSign = (Exponent > 0 ? 1 : Exponent < 0 ? -1 :
                                                                0)>
...
public:
    ...
    static FixedPoint One()
    {
        return FromScalarRaw(
            kIntegerBits == 0 ? ScalarRawMax() : (ScalarRawType(1) << (kIntegerBits == 0 ? 0 : kFractionalBits)));
    }
