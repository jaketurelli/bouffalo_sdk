...
static char *prepbuffsize(luaL_Buffer *B, size_t sz, int boxidx)
{
    ...
    if (B->size - B->n >= sz) /* enough space? */
        ...
        if (buffonstack(B))                                  /* buffer already has a box? */
            ...
            luaport_memcpy(newbuff, B->b, B->n * sizeof(char)); /* copy original content */
        }
        B->b = newbuff;
        B->size = newsize;
        return newbuff + B->n;
...
LUALIB_API void luaL_addlstring(luaL_Buffer *B, const char *s, size_t l)
{
    if (l > 0) { /* avoid 'memcpy' when 's' can be NULL */
        ...
        luaport_memcpy(b, s, l * sizeof(char));
        luaL_addsize(B, l);
    }
}
...
LUALIB_API void luaL_addvalue(luaL_Buffer *B)
{
    ...
    luaport_memcpy(b, s, len * sizeof(char));
    luaL_addsize(B, len);
...
static const char *getF(lua_State *L, void *ud, size_t *size)
{
    ...
    if (lf->n > 0) {   /* are there pre-read characters to be read? */
       ...
       The next check avoids this problem. */
        if (luaport_feof(lf->f))
            return NULL;
        *size = luaport_fread(lf->buff, 1, sizeof(lf->buff), lf->f); /* read block */
    }
    return lf->buff;
}
...
static int skipBOM(LoadF *lf)
{
    ...
    return luaport_getc(lf->f); /* return next character */
}
...
static int skipcomment(LoadF *lf, int *cp)
{
    ...
    if (c == '#') { /* first line is a comment (Unix exec. file)? */
        ...
        *cp = luaport_getc(lf->f); /* skip end-of-line, if present */
                              ...
                              const char *mode)
{
    ...
    readstatus = luaport_ferror(lf.f);
    if (filename)
        luaport_fclose(lf.f); /* close file (even in case of errors) */
    if (readstatus) {
...
static void *l_alloc(void *ud, void *ptr, size_t osize, size_t nsize)
{
    ...
    if (nsize == 0) {
        ...
        return luaport_realloc(ptr, nsize);
}
