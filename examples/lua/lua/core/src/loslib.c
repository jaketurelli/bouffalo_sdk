...
static int os_remove(lua_State *L)
{
    ...
    return luaL_fileresult(L, luaport_remove(filename) == 0, filename);
}
...
static int os_rename(lua_State *L)
{
    ...
    return luaL_fileresult(L, luaport_rename(fromname, toname) == 0, NULL);
}
...
{
    lua_pushstring(L, luaport_getenv(luaL_checkstring(L, 1))); /* if NULL push nil */
    return 1;
}

static int os_clock(lua_State *L)
{
    lua_pushnumber(L, ((lua_Number)luaport_clock()) / (lua_Number)CLOCKS_PER_SEC);
    return 1;
}
...
static int os_exit(lua_State *L)
{
    ...
    if (lua_toboolean(L, 2))
        lua_close(L);
    if (L)
        luaport_exit(status); /* 'if' to avoid warnings for unreachable 'return' */
    return 0;
}
