...
static int str_rep(lua_State *L)
{
    ...
    if (n <= 0)
        ...
        while (n-- > 1) { /* first n-1 copies (followed by separator) */
            luaport_memcpy(p, s, l * sizeof(char));
            p += l;
            if (lsep > 0) { /* empty 'memcpy' is not that cheap */
                luaport_memcpy(p, sep, lsep * sizeof(char));
                p += lsep;
            }
        }
        luaport_memcpy(p, s, l * sizeof(char)); /* last copy (not followed by separator) */
                             ...
                             char *form)
{
    ...
    *(form++) = '%';
    luaport_memcpy(form, strfrmt, len * sizeof(char));
    *(form + len) = '\0';
