...
*/
__STATIC_INLINE void csi_dcache_invalid_range(void *addr, int64_t dsize)
{
#if (__DCACHE_PRESENT == 1U)
...
/**
  ...
  \param[in]   dsize   size of memory block (in number of bytes)
*/
__STATIC_INLINE void csi_dcache_clean_range(void *addr, int64_t dsize)
{
...
*/
__STATIC_INLINE void csi_dcache_clean_invalid_range(void *addr, int64_t dsize)
{
#if (__DCACHE_PRESENT == 1U)
    int64_t op_size = dsize + (uint64_t)addr % 64;
    uint64_t op_addr = (uint64_t)addr & CACHE_INV_ADDR_Msk;
